...
<template>
  <v-card class="d-flex flex-column" v-if="card !== undefined" id="card" :elevation="elv" @mouseover="elv = 10" @mouseout="elv = 5" :color="getColor()">
    <template v-if="card">
      <div class="d-flex">
      ...
      <div class="d-flex mt-auto">
        <template v-if="type === 'hand'">
          <v-btn v-if="canCombine" :disabled="model" variant="flat" @click="requestHandCombine()" color="yellow">三连</v-btn>
          <v-btn v-else :disabled="model" variant="text" @click="requestEnter()">进场</v-btn>
          <v-btn :disabled="model" variant="text" @click="requestSell()">出售</v-btn>
        </template>
        <template v-else-if="type === 'store'">
          <v-btn v-if="canCombine" :disabled="model" variant="flat" @click="requestCombine()" color="yellow">三连</v-btn>
          <v-btn v-else :disabled="model || global.player?.mine as number < 3" variant="text" @click="requestBuy()">购买</v-btn>
          <v-btn :disabled="model || global.player?.mine as number < 3" variant="text" @click="requestCache()">暂存</v-btn>
        </template>
      </div>
    </template>
  </v-card>
