...
<template>
  ...
  >
    ...
    <div class="d-flex mt-auto">
      <v-btn
        class="ml-4 mr-4 mb-4 align-self-end"
        :disabled="card.attr.gold"
        @click="isGold = !isGold"
        :color="isGold ? 'yellow' : 'white'"
        >三连</v-btn
      >
      <v-btn
        class="mb-4 align-self-end"
        v-if="card.rmrk"
        @click="showRmrk = !showRmrk"
        >{{ showRmrk ? '描述' : '备注' }}</v-btn
      >
      <div class="d-flex flex-column ml-auto mr-2 mb-2 align-self-end">
        <div
          v-for="(u, i) in Object.keys(card.unit)"
          :key="`unit-${i}`"
          class="d-flex"
        >
